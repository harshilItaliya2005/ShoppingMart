...
class MensAdapter( private var products: List<ProductData>): RecyclerView.Adapter<MensAdapter.MensViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int,
    ): MensViewHolder {
        val binding = ItemProductBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MensViewHolder(binding)
    }
    @SuppressLint("SetTextI18n")
    override fun onBindViewHolder(
        holder: MensViewHolder,
        position: Int,
    ) {
    ...
    inner class MensViewHolder(val binding: ItemProductBinding) :
